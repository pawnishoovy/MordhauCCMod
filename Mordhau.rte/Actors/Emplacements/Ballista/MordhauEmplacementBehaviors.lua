...
MordhauEmplacementBehaviors = {}

function MordhauEmplacementBehaviors.LookForTargets(AI, Owner)
...
function MordhauEmplacementBehaviors.Sentry(AI, Owner, Abort)
	local sweepUp = true
	...
	if Owner.HFlipped ~= AI.SentryFacing then
		Owner.HFlipped = AI.SentryFacing	-- turn to the direction we have been order to Mordhau
...
function MordhauEmplacementBehaviors.GoToWpt(AI, Owner, Abort)
	if not Owner.MOMoveTarget then
		if SceneMan:ShortestDistance(Owner:GetLastAIWaypoint(), Owner.Pos, false).Largest < Owner.Height * 0.15 then
			...
			if Owner.AIMode == Actor.AIMODE_GOTO then
				AI.SentryFacing = Owner.HFlipped	-- Mordhau this direction
				AI.SentryPos = Vector(Owner.Pos.X, Owner.Pos.Y) -- Mordhau this point
...
function MordhauEmplacementBehaviors.ShootTarget(AI, Owner, Abort)
...
function MordhauEmplacementBehaviors.ShootArea(AI, Owner, Abort)
...
local Proxy = {}
local Mt = {
	__index = MordhauEmplacementBehaviors,
	__newindex = function(Table, k, v)
		error("The MordhauEmplacementBehaviors table is read-only.", 2)
	end
}
setmetatable(Proxy, Mt)
MordhauEmplacementBehaviors = Proxy
