...
require("Actors/Emplacements/Catapult/MordhauEmplacementBehaviors")
require("Actors/Emplacements/Catapult/MordhauEmplacementStaticBehaviors")

NativeMordhauEmplacementStaticAI = {}

function NativeMordhauEmplacementStaticAI:Create(Owner)
	local Members = {}
...
function NativeMordhauEmplacementStaticAI:Update(Owner)
...
function NativeMordhauEmplacementStaticAI:Destroy(Owner)
	-- Tell the coroutines to abort to avoid memory leaks
	if self.Behavior then
		local msg, done = coroutine.resume(self.Behavior, self, Owner, true)
	end
...
function NativeMordhauEmplacementStaticAI:CreatePatrolBehavior(Owner)
	self.NextBehavior = coroutine.create(TurretBehaviors.Patrol)
	self.NextCleanup = nil
	self.NextBehaviorName = "Patrol"
end

function NativeMordhauEmplacementStaticAI:CreateSentryBehavior(Owner)
...
function NativeMordhauEmplacementStaticAI:CreateAttackBehavior(Owner)
...
function NativeMordhauEmplacementStaticAI:CreateSuppressBehavior(Owner)
	if Owner.FirearmIsReady then
...
function NativeMordhauEmplacementStaticAI:CreateFaceAlarmBehavior(Owner)
	self.NextBehavior = coroutine.create(HumanBehaviors.FaceAlarm)
	self.NextBehaviorName = "FaceAlarm"
	self.NextCleanup = nil
end

function NativeMordhauEmplacementStaticAI:CreatePinBehavior(Owner)
