...
function Update(self)
	...
	if self.parent == nil then
		...
		if mo and IsActor(mo) then
			self.Frame = ToActor(mo).Team
		end
		
		local leg = self:GetParent()
		if leg then
			leg = ToLeg(leg)
			self.parent = leg;
			
			if string.find(leg.PresetName, "BG")  then
				self.DrawAfterParent = false
				self.Frame = self.Frame + 4
			else
				self.DrawAfterParent = true
			end
		end

	elseif IsLeg(self.parent) then
		...
		local value = targetAngle - self.RotAngle + math.pi * 0.5 * self.FlipFactor
		local result
