...
-- function PeasantAIBehaviours.createSoundEffect(self, effectName, variations)
...
function PeasantAIBehaviours.createEmotion(self, emotion, priority, duration, canOverridePriority)
...
function PeasantAIBehaviours.createVoiceSoundEffect(self, soundContainer, priority, emotion, canOverridePriority)
	if canOverridePriority == nil then
		canOverridePriority = false;
	end
	local usingPriority
		...
		if self.voiceSound then
			if self.voiceSound:IsBeingPlayed() then
				if self.lastPriority <= usingPriority then
					if emotion then
						PeasantAIBehaviours.createEmotion(self, emotion, priority, 0, canOverridePriority);
					end
...
function PeasantAIBehaviours.handleMovement(self)
	...
	if self.controller:IsState(Controller.BODY_JUMPSTART) == true and self.controller:IsState(Controller.BODY_CROUCH) == false and self.jumpTimer:IsPastSimMS(self.jumpDelay) and not self.isJumping then
		if (self:IsPlayerControlled() and self.feetContact[1] == true or self.feetContact[2] == true) or self.wasInAir == false then
			...
			if self.Health < 10 then
				PeasantAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.effortSerious, 2, 0);
			elseif self.Health < 35 then
				PeasantAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.effortMedium, 2, 0);
			else
				PeasantAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.effortLight, 2, 0);
			end
	...
	--isSprinting
	--aiSprint = not self.MeleeAI.active == true and not self:IsPlayerControlled() and (self.controller:IsState(Controller.MOVE_LEFT) == true or self.controller:IsState(Controller.MOVE_RIGHT) == true)
	aiSprint = not self:IsPlayerControlled() and (self.controller:IsState(Controller.MOVE_LEFT) == true or self.controller:IsState(Controller.MOVE_RIGHT) == true)
...
function PeasantAIBehaviours.handleHealth(self)
	...
	if (healthTimerReady or wasLightlyInjured or wasInjured or wasHeavilyInjured) then
		...
		if self:NumberValueExists("Death By Fire") then
			...
			PeasantAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.Scream, 16, 5);
		end
		...
		if not (self.FGArm) and (self.FGArmLost ~= true) then
			...
			if self.Health < 20 then
				PeasantAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.Scream, 5, 5);
			else
				PeasantAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.Losing, 5, 5);
			end
		end
		...
		if wasHeavilyInjured then
			if math.random(0, 100) < 50 then -- just don't be in pain sometimes
				PeasantAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.Pain, 5, 2)
			end
			self.Suppression = self.Suppression + 100;
		...
		elseif wasLightlyInjured then
			...
			PeasantAIBehaviours.createEmotion(self, 2, 1, 500);
		...
		if (wasInjured or wasHeavilyInjured) and self.Head then
			...
			if self.Health > 0 then
				...
				PeasantAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.Death, 15, 5)
...
function PeasantAIBehaviours.handleSuppression(self)
	...
	-- if (blinkTimerReady) and (not self.Suppressed) and self.Head then
		-- if self.Head.Frame == self.baseHeadFrame then
			-- PeasantAIBehaviours.createEmotion(self, 1, 0, 100);
				...
				if self.suppressedVoicelineTimer:IsPastSimMS(self.suppressedVoicelineDelay) and (not self.voiceSound:IsBeingPlayed()) then
					...
					if self.Suppression > 99 then
						-- keep playing voicelines if we keep being suppressed
							PeasantAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.suppressedSerious, 6, 4);
					...
					elseif self.Suppression > 55 then
						if self.Health < 20 and math.random(0, 100) < 20 then
							PeasantAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.Losing, 5, 4);
						else
							PeasantAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.suppressedMedium, 5, 4);
						end
						self.suppressedVoicelineTimer:Reset();
						self.suppressionUpdates = 0;
					end
					if self.Suppressed == false then -- initial voiceline
						if self.Suppression > 55 then
							...
							PeasantAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.suppressedLight, 5, 4);
						end
						self.suppressedVoicelineTimer:Reset();
					end
				end
...
function PeasantAIBehaviours.handleAITargetLogic(self)
	-- SPOT ENEMY REACTION
		...
		if self.spotAllowed ~= false then
			...
			if self.LastTargetID == -1 then
				...
				then
					...
					PeasantAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.Spot, 3, 2);
...
function PeasantAIBehaviours.handleVoicelines(self)

	if self:NumberValueExists("Mordhau Invalid") then
		self:RemoveNumberValue("Mordhau Invalid");
		PeasantAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.Invalid, 4, 3);
	end
	...
	if (self:IsPlayerControlled() and UInputMan:KeyPressed(24)) then
		PeasantAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.Warcry, 6, 4);
	...
	elseif self:NumberValueExists("Warcry Together") then
		if not self.inCombat then
			...
			elseif self:NumberValueExists("Ye Olde Defeat") then
				PeasantAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.Losing, 6, 4);
			end
	...
	if self:NumberValueExists("Catapulted") then
		...
		PeasantAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.Warcry, 5, 4);
	end
	
	if self:NumberValueExists("Blocked Mordhau") then
		self:RemoveNumberValue("Blocked Mordhau");
		self.Suppression = self.Suppression + 3;
		PeasantAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.Pain, 2, 0);
	...
	if (self.attackSuccess == true) and (not self.voiceSound:IsBeingPlayed() or self.attackSuccessTimer:IsPastSimMS(self.attackSuccessTime)) then
		...
		PeasantAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.Happy, 3, 3);
	end
	...
	if self:NumberValueExists("Puglism Attack") then
		...
		if (IsHDFirearm(self.EquippedItem)) then
			...
			elseif (self.EquippedItem:IsInGroup("Weapons - Mordhau Melee") or ToMOSRotating(self.EquippedItem):NumberValueExists("Weapons - Mordhau Melee")) then
				...
				elseif self:NumberValueExists("Large Attack") then
					...
					PeasantAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.attackLarge, 3, 3);
				...
				elseif self:NumberValueExists("Extreme Attack") then
					...
					PeasantAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.Warcry, 3, 3);
		...
		if IsThrownDevice(self.EquippedItem) then
			...
			if (activated) then
				...
				-- if (self.throwGrenadeVoicelinePlayed ~= true) then
					-- --PeasantAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.throwGrenade, 3, 4);
	...
	-- if self:NumberValueExists("Scrappers Friendly Down") then
		...
		-- if self.friendlyDownTimer:IsPastSimMS(self.friendlyDownDelay) then
			...
			-- PeasantAIBehaviours.createVoiceSoundEffect(self, Sounds, 4, 4, true);		
...
function PeasantAIBehaviours.handleDying(self)
	...
	if self.Head then
		...
		if self.deathSoundPlayed ~= true then
			...
			if math.random(1, 100) < 80 then
				PeasantAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.Pain, 15, 5)
			else
				PeasantAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.Death, 15, 5)
			end
			-- for actor in MovableMan.Actors do
...
function PeasantAIBehaviours.handleRagdoll(self)
...
function PeasantAIBehaviours.handleHeadFrames(self)
...
function PeasantAIBehaviours.handleHeadLoss(self)
