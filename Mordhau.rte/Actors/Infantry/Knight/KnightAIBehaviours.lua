...
-- function KnightAIBehaviours.createSoundEffect(self, effectName, variations)
...
function KnightAIBehaviours.createEmotion(self, emotion, priority, duration, canOverridePriority)
...
function KnightAIBehaviours.createVoiceSoundEffect(self, soundContainer, priority, emotion, canOverridePriority)
	if canOverridePriority == nil then
		canOverridePriority = false;
	end
	local usingPriority
		...
		if self.voiceSound then
			if self.voiceSound:IsBeingPlayed() then
				if self.lastPriority <= usingPriority then
					if emotion then
						KnightAIBehaviours.createEmotion(self, emotion, priority, 0, canOverridePriority);
					end
	...
	if self.controller:IsState(Controller.BODY_JUMPSTART) == true and self.controller:IsState(Controller.BODY_CROUCH) == false and self.jumpTimer:IsPastSimMS(self.jumpDelay) and not self.isJumping then
		if (self:IsPlayerControlled() and self.feetContact[1] == true or self.feetContact[2] == true) or self.wasInAir == false then
			...
			if self.Health < 10 then
				KnightAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.effortSerious, 2, 0);
			elseif self.Health < 35 then
				KnightAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.effortMedium, 2, 0);
			else
				KnightAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.effortLight, 2, 0);
			end
...
function KnightAIBehaviours.handleHealth(self)
	...
	if (healthTimerReady or wasLightlyInjured or wasInjured or wasHeavilyInjured) then
		...
		if not (self.FGArm) and (self.FGArmLost ~= true) then
			...
			if self.Health < 20 then
				KnightAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.Scream, 5, 5);
			else
				KnightAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.lostArm, 5, 5);
			end
		end
		...
		if not (self.FGLeg) and (self.FGLegLost ~= true) then
			...
			if self.Health < 20 then
				...
				KnightAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.lostLeg, 5, 5);
			end
		end
		...
		if wasHeavilyInjured then
			if math.random(0, 100) < 50 then -- just don't be in pain sometimes
				if self.Health < 5 then
					KnightAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.Disoriented, 3, 2)
				else
					KnightAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.painSerious, 5, 2)
				end
			end
			self.Suppression = self.Suppression + 100;
		elseif wasInjured then
			if math.random(0, 100) < 50 then -- just don't be in pain sometimes
				if self.Health < 25 then
					KnightAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.painGasp, 3, 2)
				else
					KnightAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.painMedium, 3, 2)
				end
			end
			...
			if math.random(0, 100) < 50 then -- just don't be in pain sometimes
				if self.inCombat == true then
					if math.random(0, 100) < 50 then
						KnightAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.painLight, 5, 2)
					else
						KnightAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.painSpoken, 5, 2)
					end
				else
					KnightAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.painLight, 2, 2)
				end
			end
			KnightAIBehaviours.createEmotion(self, 2, 1, 500);
		...
		if (wasInjured or wasHeavilyInjured) and self.Head then
			...
			if self.Health > 0 then
				...
				if math.random(0, 100) < 50 then
					KnightAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.painSerious, 10, 5)
				else
					KnightAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.deathEpic, 10, 5)
				end
...
function KnightAIBehaviours.handleSuppression(self)
	...
	-- if (blinkTimerReady) and (not self.Suppressed) and self.Head then
		-- if self.Head.Frame == self.baseHeadFrame then
			-- KnightAIBehaviours.createEmotion(self, 1, 0, 100);
				...
				if self.suppressedVoicelineTimer:IsPastSimMS(self.suppressedVoicelineDelay) and (not self.voiceSound:IsBeingPlayed()) then
					...
					if self.Suppression > 99 then
						-- keep playing voicelines if we keep being suppressed
							KnightAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.suppressedSerious, 6, 4);
					...
					elseif self.Suppression > 55 then
						if self.Health < 20 and math.random(0, 100) < 20 then
							KnightAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.Losing, 5, 4);
						else
							KnightAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.suppressedMedium, 5, 4);
						end
						self.suppressedVoicelineTimer:Reset();
						self.suppressionUpdates = 0;
					end
					if self.Suppressed == false then -- initial voiceline
						if self.Suppression > 55 then
							...
							KnightAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.suppressedLight, 5, 4);
						end
						self.suppressedVoicelineTimer:Reset();
					end
				end
...
function KnightAIBehaviours.handleAITargetLogic(self)
	-- SPOT ENEMY REACTION
	...
	if (not self:IsPlayerControlled()) and self.AI.Target and IsAHuman(self.AI.Target) then
				...
				then
					...
					if isClose then
						if self.Suppressed then
							KnightAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.frustratedSerious, 4, 2);
						else
							if self.Health < 50 then
								KnightAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.whoaMedium, 4, 2);
							else
								KnightAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.frustratedMedium, 4, 2);
							end
						end
					else
						KnightAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.Spot, 3, 2);
					end
		...
		if self.combatExitTimer:IsPastSimMS(self.combatExitDelay) and self.inCombat == true then
			...
			if self.Health < 10 then
				KnightAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.recoveryExtreme, 2, 0);
			elseif self.Health < 35 then
				KnightAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.recoverySerious, 2, 0);
			elseif self.Health < 60 then
				KnightAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.recoveryMedium, 2, 0);
			elseif self.Health < 90 and math.random(0, 100) < 50 then
				KnightAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.recoveryLight, 2, 0);
			end
		end
		if self.LastTargetID ~= -1 then
			self.LastTargetID = -1
...
function KnightAIBehaviours.handleVoicelines(self)
	...
	if self:NumberValueExists("Horse Response") then
		...
		if self.horseResponseTimer:IsPastSimMS(self.horseResponseDelay) then
			KnightAIBehaviours.createVoiceSoundEffect(self, self.horseResponseTable[self:GetNumberValue("Horse Response")], 4, 3);
	...
	if self:NumberValueExists("Catapulted") then
		...
		elseif randomChance > 50 then
			CrusaderAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.Rage, 6, 4);
		elseif randomChance > 25 then
			CrusaderAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.frustratedSerious, 6, 4);
		else
	...
	if self.Victory then
		KnightAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.Victory, 5, 4);
		if self.EquippedItem and (self.EquippedItem:IsInGroup("Weapons - Mordhau Melee") or ToMOSRotating(self.EquippedItem):NumberValueExists("Weapons - Mordhau Melee")) then
			ToHDFirearm(self.EquippedItem):SetNumberValue("Warcried", 1);
		end
	end
	...
	if (self.attackSuccess == true) and (not self.voiceSound:IsBeingPlayed() or self.attackSuccessTimer:IsPastSimMS(self.attackSuccessTime)) then
		...
		KnightAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.Happy, 3, 3);
	end
		...
		if (IsHDFirearm(self.EquippedItem)) then
			...
			if self.EquippedItem:IsInGroup("Weapons - Primary") and not ToMOSRotating(self.EquippedItem):NumberValueExists("Weapons - Mordhau Melee") then
				...
				if gun.FullAuto == true and gunMag and gunMag.Capacity > 40  and gun:IsActivated() then
					...
					if self.gunShotCounter > (gunMag.Capacity*0.7) and self.suppressingVoicelineTimer:IsPastSimMS(self.suppressingVoicelineDelay) then
						KnightAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.Suppressing, 3, 3);
			...
			elseif (self.EquippedItem:IsInGroup("Weapons - Mordhau Melee") or ToMOSRotating(self.EquippedItem):NumberValueExists("Weapons - Mordhau Melee")) then
				...
				elseif self:NumberValueExists("Medium Attack") then
					...
					KnightAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.attackMedium, 3, 3);
				...
				elseif self:NumberValueExists("Large Attack") then
					...
					KnightAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.attackLarge, 3, 3);
		...
		if IsThrownDevice(self.EquippedItem) then
			...
			if (activated) then
				...
				-- if (self.throwGrenadeVoicelinePlayed ~= true) then
					-- --KnightAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.throwGrenade, 3, 4);
	...
	-- if self:NumberValueExists("Scrappers Friendly Down") then
		...
		-- if self.friendlyDownTimer:IsPastSimMS(self.friendlyDownDelay) then
			...
			-- KnightAIBehaviours.createVoiceSoundEffect(self, Sounds, 4, 4, true);		
...
function KnightAIBehaviours.handleDying(self)
	...
	if self.Head then
		...
		if self.deathSoundPlayed ~= true then
			...
			if math.random(1, 100) < 80 then
				KnightAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.painSerious, 15, 5)
			else
				KnightAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.deathEpic, 15, 5)
			end
			-- for actor in MovableMan.Actors do
				-- if actor.Team == self.Team then
					-- local d = SceneMan:ShortestDistance(actor.Pos, self.Pos, true).Magnitude;
			...
			if not (self.voiceSound) or (not self.voiceSound:IsBeingPlayed()) then
				...
				if self.inCombat == true and (math.random(1, 100) < self.incapacitationChance) then
					if self.seriousDeath == true then
						KnightAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.Incap, 14, 2)
					else
						KnightAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.incapSpoken, 14, 2)
					end
...
function KnightAIBehaviours.handleRagdoll(self)
...
function KnightAIBehaviours.handleHeadFrames(self)
...
function KnightAIBehaviours.handleHeadLoss(self)
