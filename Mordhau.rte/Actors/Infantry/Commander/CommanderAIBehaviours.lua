CommanderAIBehaviours = {};

-- function CommanderAIBehaviours.createSoundEffect(self, effectName, variations)
...
function CommanderAIBehaviours.createEmotion(self, emotion, priority, duration, canOverridePriority)
...
function CommanderAIBehaviours.createVoiceSoundEffect(self, soundContainer, priority, emotion, canOverridePriority)
	if canOverridePriority == nil then
		canOverridePriority = false;
	end
	local usingPriority
		...
		if self.voiceSound then
			if self.voiceSound:IsBeingPlayed() then
				if self.lastPriority <= usingPriority then
					if emotion then
						CommanderAIBehaviours.createEmotion(self, emotion, priority, 0, canOverridePriority);
					end
					self.voiceSound:Stop();
					if type(soundContainer) == "table" then
						local workingSoundContainer = soundContainer[math.random(1, #soundContainer)];
						self.voiceSound = workingSoundContainer;
						workingSoundContainer:Play(self.Pos);
					else		
...
function CommanderAIBehaviours.handleMovement(self)
	...
	if self.controller:IsState(Controller.BODY_JUMPSTART) == true and self.controller:IsState(Controller.BODY_CROUCH) == false and self.jumpTimer:IsPastSimMS(self.jumpDelay) and not self.isJumping then
		if (self:IsPlayerControlled() and self.feetContact[1] == true or self.feetContact[2] == true) or self.wasInAir == false then
			...
			if self.Health < 10 then
				CommanderAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.effortSerious, 2, 0);
			elseif self.Health < 35 then
				CommanderAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.effortMedium, 2, 0);
			else
				CommanderAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.effortLight, 2, 0);
			end
...
function CommanderAIBehaviours.handleHealth(self)

	local healthTimerReady = self.healthUpdateTimer:IsPastSimMS(750);
	local wasLightlyInjured = self.Health < (self.oldHealth - 3);
	local wasInjured = self.Health < (self.oldHealth - 15);
	local wasHeavilyInjured = self.Health < (self.oldHealth - 25);
	...
	if (healthTimerReady or wasLightlyInjured or wasInjured or wasHeavilyInjured) then
		...
		if not (self.FGArm) and (self.FGArmLost ~= true) then
			...
			if self.Health < 20 then
				CommanderAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.Scream, 5, 5);
			end
		end
		...
		if wasHeavilyInjured then
			if math.random(0, 100) < 50 then -- just don't be in pain sometimes
				CommanderAIBehaviours.createVoiceSoundEffect(self, self.atmosphericVoiceSounds.painSerious, 5, 2)
			end
			self.Suppression = self.Suppression + 100;
		elseif wasInjured then
			if math.random(0, 100) < 50 then -- just don't be in pain sometimes
				CommanderAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.painMedium, 3, 2)
			end
			self.Suppression = self.Suppression + 50;
		elseif wasLightlyInjured then
			if math.random(0, 100) < 50 then -- just don't be in pain sometimes
				CommanderAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.painLight, 2, 2)
			end
			CommanderAIBehaviours.createEmotion(self, 2, 1, 500);
		...
		if (wasInjured or wasHeavilyInjured) and self.Head then
			...
			if self.Health > 0 then
				...
				if math.random(0, 100) < 50 then
					CommanderAIBehaviours.createVoiceSoundEffect(self, self.atmosphericVoiceSounds.painSerious, 10, 5)
				else
					CommanderAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.deathEpic, 10, 5)
				end
...
function CommanderAIBehaviours.handleSuppression(self)
	...
	-- if (blinkTimerReady) and (not self.Suppressed) and self.Head then
		-- if self.Head.Frame == self.baseHeadFrame then
			-- CommanderAIBehaviours.createEmotion(self, 1, 0, 100);
				...
				if self.suppressedVoicelineTimer:IsPastSimMS(self.suppressedVoicelineDelay) and (not self.voiceSound:IsBeingPlayed()) then
					...
					if self.Suppression > 99 then
						-- keep playing voicelines if we keep being suppressed
							CommanderAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.suppressedSerious, 6, 4);
						self.suppressedVoicelineTimer:Reset();
						self.suppressionUpdates = 0;
					elseif self.Suppression > 55 then
						CommanderAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.suppressedMedium, 5, 4);
						self.suppressedVoicelineTimer:Reset();
						self.suppressionUpdates = 0;
					end
					if self.Suppressed == false then -- initial voiceline
						if self.Suppression > 55 then
							...
							CommanderAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.suppressedLight, 5, 4);
						end
						self.suppressedVoicelineTimer:Reset();
					end
				end
...
function CommanderAIBehaviours.handleAITargetLogic(self)
	-- SPOT ENEMY REACTION
	...
	if (not self:IsPlayerControlled()) and self.AI.Target and IsAHuman(self.AI.Target) then
				...
				then
					...
					if isClose then
						if self.Suppressed then
							CommanderAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.frustratedSerious, 4, 2);
						else
							CommanderAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.whoaMedium, 4, 2);
						end
					else
						CommanderAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.Spot, 3, 2);
					end
		...
		if self.combatExitTimer:IsPastSimMS(self.combatExitDelay) and self.inCombat == true then
			...
			if self.Health < 10 then
				CommanderAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.recoveryExtreme, 2, 0);
			elseif self.Health < 35 then
				CommanderAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.recoverySerious, 2, 0);
			elseif self.Health < 60 then
				CommanderAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.recoverySerious, 2, 0);
			elseif self.Health < 90 and math.random(0, 100) < 50 then
				CommanderAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.recoveryLight, 2, 0);
			end
		end
		if self.LastTargetID ~= -1 then
			self.LastTargetID = -1
...
function CommanderAIBehaviours.handleVoicelines(self)
	...
	if self:NumberValueExists("Mordhau Arrow Suppression") then
		...
		CommanderAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.Archers, 3, 0);
	end
	...
	if self:NumberValueExists("Blocked Mordhau") then
		...
		self.Suppression = self.Suppression + 3;
		if self.Suppressed then
			CommanderAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.whoaLight, 2, 0);
	...
	if (self.attackKilled == true) and (not self.voiceSound:IsBeingPlayed() or self.attackKilledTimer:IsPastSimMS(self.attackKilledTime)) then
		...
		CommanderAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.killingEnemy, 5, 4);
	end
	
	if self:NumberValueExists("Mordhau Charge") then
		self:RemoveNumberValue("Mordhau Charge");
		CommanderAIBehaviours.createVoiceSoundEffect(self, self.atmosphericVoiceSounds.battleScream, 3, 3);
	...
	if self:NumberValueExists("Puglism Block") then
		...
		KnightAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.gruntLight, 2, 0);
		...
		if (IsHDFirearm(self.EquippedItem)) then
			...
			elseif (self.EquippedItem:IsInGroup("Weapons - Mordhau Melee") or ToMOSRotating(self.EquippedItem):NumberValueExists("Weapons - Mordhau Melee")) then
				...
				elseif self:NumberValueExists("Medium Attack") then
					...
					CommanderAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.gruntMedium, 3, 3);
					self.movementSounds.AttackMedium:Play(self.Pos);
				elseif self:NumberValueExists("Large Attack") then
					self:RemoveNumberValue("Large Attack");
					CommanderAIBehaviours.createVoiceSoundEffect(self, self.atmosphericVoiceSounds.gruntSerious, 3, 3);
		...
		if IsThrownDevice(self.EquippedItem) then
			...
			if (activated) then
				...
				-- if (self.throwGrenadeVoicelinePlayed ~= true) then
					-- --CommanderAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.throwGrenade, 3, 4);
	...
	-- if self:NumberValueExists("Scrappers Friendly Down") then
		...
		-- if self.friendlyDownTimer:IsPastSimMS(self.friendlyDownDelay) then
			...
			-- CommanderAIBehaviours.createVoiceSoundEffect(self, Sounds, 4, 4, true);		
...
function CommanderAIBehaviours.handleDying(self)
	...
	if self.Head then
		...
		if self.deathSoundPlayed ~= true then
			...
			if math.random(1, 100) < 80 then
				CommanderAIBehaviours.createVoiceSoundEffect(self, self.atmosphericVoiceSounds.painSerious, 15, 5)
			else
				CommanderAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.deathEpic, 15, 5)
			end
			-- for actor in MovableMan.Actors do
				-- if actor.Team == self.Team then
					-- local d = SceneMan:ShortestDistance(actor.Pos, self.Pos, true).Magnitude;
			...
			if not (self.voiceSound) or (not self.voiceSound:IsBeingPlayed()) then
				...
				if self.inCombat == true and (math.random(1, 100) < self.incapacitationChance) then
					if self.seriousDeath == true then
						CommanderAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.Incap, 14, 2)
					end
...
function CommanderAIBehaviours.handleRagdoll(self)
...
function CommanderAIBehaviours.handleHeadFrames(self)
...
function CommanderAIBehaviours.handleHeadLoss(self)
