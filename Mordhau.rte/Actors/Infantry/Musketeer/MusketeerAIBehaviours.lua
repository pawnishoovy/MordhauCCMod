MusketeerAIBehaviours = {};

-- function MusketeerAIBehaviours.createSoundEffect(self, effectName, variations)
...
function MusketeerAIBehaviours.createEmotion(self, emotion, priority, duration, canOverridePriority)
...
function MusketeerAIBehaviours.createVoiceSoundEffect(self, soundContainer, priority, emotion, canOverridePriority)
	if canOverridePriority == nil then
		canOverridePriority = false;
	end
	local usingPriority
		...
		if self.voiceSound then
			if self.voiceSound:IsBeingPlayed() then
				if self.lastPriority <= usingPriority then
					if emotion then
						MusketeerAIBehaviours.createEmotion(self, emotion, priority, 0, canOverridePriority);
					end
...
function MusketeerAIBehaviours.handleMovement(self)
	...
	if self.controller:IsState(Controller.BODY_JUMPSTART) == true and self.controller:IsState(Controller.BODY_CROUCH) == false and self.jumpTimer:IsPastSimMS(self.jumpDelay) and not self.isJumping then
		if (self:IsPlayerControlled() and self.feetContact[1] == true or self.feetContact[2] == true) or self.wasInAir == false then
			...
			if self.Health < 10 then
				MusketeerAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.effortSerious, 2, 0);
			elseif self.Health < 35 then
				MusketeerAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.effortMedium, 2, 0);
			else
				MusketeerAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.effortLight, 2, 0);
			end
...
function MusketeerAIBehaviours.handleHealth(self)
	...
	if (healthTimerReady or wasLightlyInjured or wasInjured or wasHeavilyInjured) then
		...
		if not (self.FGArm) and (self.FGArmLost ~= true) then
			...
			if self.Health < 20 then
				MusketeerAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.Scream, 5, 5);
			else
				MusketeerAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.lostArm, 5, 5);
			end
		end
		...
		if not (self.FGLeg) and (self.FGLegLost ~= true) then
			...
			if self.Health < 20 then
				...
				MusketeerAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.lostLeg, 5, 5);
			end
		end
		...
		if wasHeavilyInjured then
			if math.random(0, 100) < 50 then -- just don't be in pain sometimes
				if self.Health < 5 then
					MusketeerAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.Disoriented, 5, 2)
				else
					MusketeerAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.painSerious, 5, 2)
				end
			end
			self.Suppression = self.Suppression + 100;
		elseif wasInjured then
			if math.random(0, 100) < 50 then -- just don't be in pain sometimes
				if self.Health < 25 then
					MusketeerAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.painGasp, 5, 2)
				else
					MusketeerAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.painMedium, 5, 2)
				end
			end
			...
			if math.random(0, 100) < 50 then -- just don't be in pain sometimes
				if self.inCombat == true then
					if math.random(0, 100) < 50 then
						MusketeerAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.painLight, 5, 2)
					else
						MusketeerAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.painSpoken, 5, 2)
					end
				else
					MusketeerAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.painLight, 2, 2)
				end
			end
			MusketeerAIBehaviours.createEmotion(self, 2, 1, 500);
...
function MusketeerAIBehaviours.handleSuppression(self)
	...
	-- if (blinkTimerReady) and (not self.Suppressed) and self.Head then
		-- if self.Head.Frame == self.baseHeadFrame then
			-- MusketeerAIBehaviours.createEmotion(self, 1, 0, 100);
				...
				if self.suppressedVoicelineTimer:IsPastSimMS(self.suppressedVoicelineDelay) and (not self.voiceSound:IsBeingPlayed()) then
					...
					if self.Suppression > 99 then
						-- keep playing voicelines if we keep being suppressed
							MusketeerAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.suppressedSerious, 6, 4);
					...
					elseif self.Suppression > 55 then
						if self.Health < 20 and math.random(0, 100) < 20 then
							MusketeerAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.Losing, 5, 4);
						else
							MusketeerAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.suppressedMedium, 5, 4);
						end
						self.suppressedVoicelineTimer:Reset();
						self.suppressionUpdates = 0;
					end
					if self.Suppressed == false then -- initial voiceline
						if self.Suppression > 55 then
							...
							MusketeerAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.suppressedLight, 5, 4);
						end
						self.suppressedVoicelineTimer:Reset();
					end
				end
...
function MusketeerAIBehaviours.handleAITargetLogic(self)
	-- SPOT ENEMY REACTION
	...
	if (not self:IsPlayerControlled()) and self.AI.Target and IsAHuman(self.AI.Target) then
				...
				then
					...
					if isClose then
						if self.Suppressed then
							MusketeerAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.frustratedSerious, 4, 2);
						else
							if self.Health < 50 then
								MusketeerAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.whoaMedium, 4, 2);
							else
								MusketeerAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.frustratedMedium, 4, 2);
							end
						end
					else
						MusketeerAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.Spot, 3, 2);
					end
		...
		if self.combatExitTimer:IsPastSimMS(self.combatExitDelay) and self.inCombat == true then
			...
			if self.Health < 10 then
				MusketeerAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.recoveryExtreme, 2, 0);
			elseif self.Health < 35 then
				MusketeerAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.recoverySerious, 2, 0);
			elseif self.Health < 60 then
				MusketeerAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.recoveryMedium, 2, 0);
			elseif self.Health < 90 and math.random(0, 100) < 50 then
				MusketeerAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.recoveryLight, 2, 0);
			end
		end
		if self.LastTargetID ~= -1 then
			self.LastTargetID = -1
...
function MusketeerAIBehaviours.handleVoicelines(self)
	...
	if (self:IsPlayerControlled() and UInputMan:KeyPressed(24)) then
		MusketeerAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.Warcry, 6, 4);
	...
	elseif self:NumberValueExists("Warcry Together") then
		if not self.inCombat then
			...
			if self:NumberValueExists("Ye Olde Victory") then
				self.Victory = true;
				MusketeerAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.victorySpoken, 6, 4);
				if self.EquippedItem and (self.EquippedItem:IsInGroup("Weapons - Mordhau Melee") or ToMOSRotating(self.EquippedItem):NumberValueExists("Weapons - Mordhau Melee")) then
					ToHDFirearm(self.EquippedItem):SetNumberValue("Warcried", 1);
				end			
			elseif self:NumberValueExists("Ye Olde Defeat") then
				MusketeerAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.Defeat, 6, 4);
			end
	...
	if self.Victory then
		MusketeerAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.Victory, 5, 4);
		if self.EquippedItem and (self.EquippedItem:IsInGroup("Weapons - Mordhau Melee") or ToMOSRotating(self.EquippedItem):NumberValueExists("Weapons - Mordhau Melee")) then
			ToHDFirearm(self.EquippedItem):SetNumberValue("Warcried", 1);
		end
	end
	...
	if self:NumberValueExists("Mordhau Arrow Suppression") then
		...
		MusketeerAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.Archers, 3, 0);
	end
	
	if self:NumberValueExists("Blocked Mordhau") then
		self:RemoveNumberValue("Blocked Mordhau");
		self.Suppression = self.Suppression + 3;
		MusketeerAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.frustratedLight, 2, 0);
		if self:NumberValueExists("Blocked Heavy Mordhau") then
			...
			MusketeerAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.frustratedMedium, 3, 0);
		end
	end
	...
	if (self.attackSuccess == true) and (not self.voiceSound:IsBeingPlayed() or self.attackSuccessTimer:IsPastSimMS(self.attackSuccessTime)) then
		...
		MusketeerAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.Happy, 3, 3);
	end
	...
	if (self.attackKilled == true) and (not self.voiceSound:IsBeingPlayed() or self.attackKilledTimer:IsPastSimMS(self.attackKilledTime)) then
		...
		MusketeerAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.killingEnemy, 5, 4);
	end
	
	if self:NumberValueExists("Kick Attack") then
		self:RemoveNumberValue("Kick Attack");
		MusketeerAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.attackLight, 3, 3);
		self.movementSounds.Jump:Play(self.Pos);
	end
	
	if self:NumberValueExists("Puglism Block") then
		self:RemoveNumberValue("Puglism Block");
		MusketeerAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.attackLight, 2, 0);
		...
		if (IsHDFirearm(self.EquippedItem)) then
			...
			if self.EquippedItem:IsInGroup("Weapons - Primary") and not ToMOSRotating(self.EquippedItem):NumberValueExists("Weapons - Mordhau Melee") then
				...
				if gun.FullAuto == true and gunMag and gunMag.Capacity > 40  and gun:IsActivated() then
					...
					if self.gunShotCounter > (gunMag.Capacity*0.7) and self.suppressingVoicelineTimer:IsPastSimMS(self.suppressingVoicelineDelay) then
						MusketeerAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.Suppressing, 3, 3);
			...
			elseif (self.EquippedItem:IsInGroup("Weapons - Mordhau Melee") or ToMOSRotating(self.EquippedItem):NumberValueExists("Weapons - Mordhau Melee")) then
				...
				elseif self:NumberValueExists("Medium Attack") then
					...
					MusketeerAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.attackMedium, 3, 3);
					self.movementSounds.AttackMedium:Play(self.Pos);
				elseif self:NumberValueExists("Large Attack") then
					self:RemoveNumberValue("Large Attack");
					MusketeerAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.attackLarge, 3, 3);
				...
				elseif self:NumberValueExists("Extreme Attack") then
					...
					MusketeerAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.attackExtreme, 3, 3);
		...
		if IsThrownDevice(self.EquippedItem) then
			...
			if (activated) then
				...
				-- if (self.throwGrenadeVoicelinePlayed ~= true) then
					-- --MusketeerAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.throwGrenade, 3, 4);
	...
	-- if self:NumberValueExists("Scrappers Friendly Down") then
		...
		-- if self.friendlyDownTimer:IsPastSimMS(self.friendlyDownDelay) then
			...
			-- MusketeerAIBehaviours.createVoiceSoundEffect(self, Sounds, 4, 4, true);		
...
function MusketeerAIBehaviours.handleDying(self)
			...
			if not (self.voiceSound) or (not self.voiceSound:IsBeingPlayed()) then
				...
				if self.inCombat == true and (math.random(1, 100) < self.incapacitationChance) then
					if self.seriousDeath == true then
						MusketeerAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.Incap, 14, 2)
					else
						MusketeerAIBehaviours.createVoiceSoundEffect(self, self.voiceSounds.incapSpoken, 14, 2)
					end
...
function MusketeerAIBehaviours.handleRagdoll(self)
...
function MusketeerAIBehaviours.handleHeadFrames(self)
...
function MusketeerAIBehaviours.handleHeadLoss(self)
